...
class ResourceRegionMessageWriter : HttpMessageWriter<ResourceRegion> {
    ...
    private val mediaTypes: List<MediaType> =
        MediaType.asMediaTypes(regionEncoder.encodableMimeTypes)

    override fun canWrite(elementType: ResolvableType, mediaType: MediaType?): Boolean =
        regionEncoder.canEncode(elementType, mediaType)

    override fun getWritableMediaTypes(): List<MediaType> = mediaTypes

    override fun write(
        inputStream: Publisher<out ResourceRegion>,
        elementType: ResolvableType,
        mediaType: MediaType?,
        message: ReactiveHttpOutputMessage,
        hints: Map<String, Any>
    ): Mono<Void> = TODO("use server implementation below")
    ...
    override fun write(
        ...
        response: ServerHttpResponse, hints: Map<String, Any>
    ): Mono<Void> {
        ...
        return Mono.from(inputStream).flatMap { resourceRegion ->
            ...
            val end = (start + resourceRegion.count - 1).coerceAtMost(contentLength - 1)
    ...
    private companion object {
        ...
        fun zeroCopy(
            resource: Resource, region: ResourceRegion,
            message: ReactiveHttpOutputMessage
        ): Optional<Mono<Void>> {
            if (message is ZeroCopyHttpOutputMessage && resource.isFile) {
                return Optional.of(message.writeWith(resource.file, region.position, region.count))
            }
